...
/**
 * Produces time-paced, seeded network events for towns, railways, and population changes.
 *
 * <p>The clock boundary is injectable so the exact event stream can be verified without waiting
 * for wall-clock time.</p>
 */
public final class RandomNetworkEventSource implements NetworkEventSource
{
    ...
    private final List<String> townNames = new ArrayList<>();
    private final List<Integer> townPopulations = new ArrayList<>();
    private final List<Long> railwayNone = new ArrayList<>();
    private final List<Long> railwaySingle = new ArrayList<>();
    private final List<Long> railwayDual = new ArrayList<>();
    private final Queue<String> messages = new ArrayDeque<>();

    private long lastTime;
    private double errorProb = SCREW_UP_P;

    public RandomNetworkEventSource(long seed)
    {
        this(seed, System::currentTimeMillis);
    }

    public RandomNetworkEventSource()
    {
        this(new Rand(), System::currentTimeMillis);
    }

    RandomNetworkEventSource(long seed, LongSupplier currentTimeMillis)
    {
        this(new Rand(seed), currentTimeMillis);
    }

    private RandomNetworkEventSource(Rand rand, LongSupplier currentTimeMillis)
    {
        this.rand = Objects.requireNonNull(rand, "rand");
        this.currentTimeMillis = Objects.requireNonNull(
                currentTimeMillis,
                "currentTimeMillis");
        this.lastTime = currentTimeMillis.getAsLong();
    }
    ...
    public void setErrorProbability(double errorProb)
    {
        if(errorProb < 0.0 || 0.5 < errorProb)
        {
            throw new IllegalArgumentException(
                    "Error probability must be between 0.0 and 0.5.");
        }
    ...
    {
        long thisTime = currentTimeMillis.getAsLong();
...
class Rand
{
    ...
    public static final CharSequence DIGIT           = range('0', '9');
    ...
    private final Set<Object> prev = new HashSet<>();
